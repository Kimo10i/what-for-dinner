        ...
        for (const mutation of mutations)
            if (mutation.type === "childList")
                for (const node of mutation.addedNodes)
                    node.tagName === "LINK" && node.rel === "modulepreload" && fetchModule(node)
    }).observe(document, {
        childList: true,
        subtree: true
    });
...
window.addEventListener("load", function() {
    ...
    function displayRecipe(recipe) {
        ...
        if (recipeImage && recipe.imgCover) {
        ...
        recipe.ingredients.forEach((ingredient, index) => {
            ...
            li.innerHTML = `
                <div class="badge-circle bg-primary rounded-circle text-white fw-bold mt-1 me-3 flex-shrink-0">${index + 1}</div>
        ...
        recipe.instructions.forEach((instruction, index) => {
            ...
            div.innerHTML = `
                <div class="badge-square-lg bg-primary rounded-4 text-white fw-bold me-4 flex-shrink-0">${index + 1}</div>
                <div class="flex-grow-1 pt-2">
                    <p class="text-secondary mb-0">${instruction}</p>
                </div>
            `;
        ...
        recipe.tips.forEach(tip => {
            const div = document.createElement("div");
            div.className = "d-flex align-items-start p-3 bg-warning-subtle rounded-3 border-start border-4 border-warning";
            div.innerHTML = `
                <i class="fa-solid fa-circle-check text-warning fs-5 me-3 mt-1"></i>
                <p class="text-dark mb-0">${tip}</p>
            `;
        ...
        if (recipe.totalTime > 45) {
            ...
            timeWarning.classList.add("d-flex");
        } else {
            timeWarning.classList.add("d-none");
            timeWarning.classList.remove("d-flex");
        }
